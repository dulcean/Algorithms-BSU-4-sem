...
private:
    ...
    size_t getMax(size_t tl, size_t tr, size_t l, size_t r, size_t v) {
        if (l > r)
            return 0;
        if (l == tl && r == tr)
            return tree[v];
        size_t mid = (tl + tr) / 2;
        return max(getMax(tl, mid, l, min(r, mid), v * 2 + 1),
            getMax(mid + 1, tr, max(l, mid + 1), r, v * 2 + 2));
    }

    void buildTree(size_t tl, size_t tr, size_t v) {
        if (tl == tr) {
            tree[v] = 0;
            return;
        }
        size_t mid = (tl + tr) / 2;
        buildTree(tl, mid, v * 2 + 1);
        buildTree(mid + 1, tr, v * 2 + 2);
        tree[v] = max(tree[v * 2 + 1], tree[v * 2 + 2]);
    }

    void updateValue(size_t tl, size_t tr, size_t v, size_t pos, ll new_val) {
        if (tl == tr) {
            tree[v] = new_val;
            return;
        }
        size_t mid = (tl + tr) / 2;
        if (pos <= mid)
            updateValue(tl, mid, v * 2 + 1, pos, new_val);
        else
            updateValue(mid + 1, tr, v * 2 + 2, pos, new_val);
        tree[v] = max(tree[v * 2 + 1], tree[v * 2 + 2]);
    }

public:
    SegmentTree(ll _size) : n(_size) {
        tree.resize(4 * n - 5);
        buildTree(0, n - 1, 0);
    }

    ll queryMax(size_t l, size_t r) {
        return getMax(0, n - 1, l, r, 0);
    }

    void updateValue(size_t pos, ll new_val) {
        updateValue(0, n - 1, 0, pos, new_val);
    }
};
...
#define endl '\n'
multimap <ll, ll> ::iterator m1_Iter;
multimap <ll, ll> ::iterator m2_Iter;


signed main() {
    ios_base::sync_with_stdio(false); cin.tie(NULL); cout.tie(NULL);

    freopen("m-gaps.in", "r", stdin);
    freopen("m-gaps.out", "w", stdout);

    int n, m;
    cin >> n >> m;
    vector<ll> arr(n);
    vector<ll> restored_array(n);
    vector<vector<ll>> dp(m + 1, vector<ll>(n, 0));
    for (int i = 0; i < n; i++) {
        dp[0][i] = 1;
    }
    multimap<ll, ll> mapka;
    for (int i = 0; i < n; i++) {
        cin >> arr[i];
        mapka.insert({ arr[i], i });
    }
    ...
    for (auto it = mapka.begin(); it != mapka.end(); ++it) {
        ...
        restored_array[it->second] = id;
    }
    ll counter = 0;
    for (int i = 0; i <= m; i++) {
        SegmentTree st((size_t)n);
        counter = 0;
        for (int j = 0; j < n; j++) {
            if (i != 0 && j != 0) {
                counter = max(counter, dp[i - 1][j - 1]);
            }
            dp[i][j] = max(
                st.queryMax
                (
            ...
            st.updateValue(restored_array[j], dp[i][j]);
        }
    }

    cout << dp[m][maxind(dp[m])];
